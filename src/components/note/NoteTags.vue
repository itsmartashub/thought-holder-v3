<template>
	<form class="note__add-tags-form" @submit.prevent @click.stop tabindex="1">
		<h3 class="h2">Tag note</h3>
		<span class="note__add-tags-closebtn" @click="emitNoteTagsOpen"
			>&#10005;</span
		>
		<input
			class="note__add-tags-input"
			type="text"
			placeholder="Enter tag name"
			v-model.trim="inputSearchOrAdd"
			@input="searchOrAddTag()"
		/>
		<ul class="note__add-tags-ul">
			<li
				class="note__add-tags-li"
				v-for="(tag, key) in GET_TAGS"
				:key="key"
			>
				<label :for="key">
					<input
			...
			<li class="mt-1" v-if="create" @click.stop="createNewTag()">
				<i class="mdi mdi-plus"></i>
				<p>
					Create tag: &nbsp; <span>{{ this.inputSearchOrAdd }}</span>
				</p>
			</li>
		</ul>
	</form>
