	...
	<main class="notes-home">
		...
		<section class="notes-home__filled " v-if="GET_NOTES.length > 0">
			...
			<transition name="scale" mode="in-out">
				<section class="notes__pinned mb-4" v-if="GET_PINNED.length > 0">
					<a href="#first-pinned-note">
						<h4 class="notes__pinned-title h4"><i class="mdi mdi-pin"></i> Pinned Notes </h4>
					</a>
					<transition-group name="scale" mode="out-in" class="notes m-auto">
			...
			<section class="notes__unpinned"><br/><br/>
				<h4 class="notes__pinned-title h4"><i class="mdi mdi-note"></i>OTHERS</h4>
					<transition-group name="scale" mode="out-in" class="notes m-auto">
		...
		<button class="new-note__float-btn" @click.prevent="mobileShowNewNote()">+</button>
	...
	export default {
		...
		methods: {
			mobileShowNewNote() {
				...
				document.querySelector('.new-note__wrapper').classList.add('new-note__wrapper--show');
			}
		}
	}
</script>